...
Quando possível, a função equivalente do OpenCV também será apresentada, já que as acelerações de GPU podem ser facilmente acessadas usando \pyline{cv2.cuda} em vez de apenas \pyline{cv2} \autocite{ref:cvcuda}. Todas as operações também podem ser feitas por uma \textit{lookup table}, com a função \pyline{cv2.LUT} \autocite{ref:LUT}. Entretanto, as funções deste trabalho foram implementadas apenas com Numpy, visando a familiarização com técnicas de vetorização.
...
\input{impl/mono}
\input{impl/negativo}
\input{impl/espvertical}
\input{impl/convint}
\input{impl/invpares}
\input{impl/reflexao}
\input{impl/ajgama}
\input{impl/planobit}
\input{impl/combinacao}
\input{impl/mosaico}
